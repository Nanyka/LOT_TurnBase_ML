...
{
    public class AoeEntityInfoUI : MonoBehaviour
    {
        ...
        private void ShowUnitInfo(IShowInfo infoGetter)
        {
            ...
            if (_selectedEntity.TryGetComponent(out IGetEntityData<BuildingStats> buildingData))
            {
                var buildingItem =
                    SavingSystemManager.Instance.GetInventoryItemByName(buildingData.GetData().EntityName);
                entityIcon.sprite = AddressableManager.Instance.GetAddressableSprite(buildingItem.spriteAddress);
                hpSlider.value = buildingData.GetData().CurrentHp;
                strengthSlider.value = buildingData.GetStats().AttackDamage;
                defendSlider.value = buildingData.GetStats().Shield;
                foreach (var skill in skills)
                    skill.Deactivate();
                
                infoMenu.SetActive(true);
                
                var buildingInGame = _selectedEntity.GetComponent<IConfirmFunction>();
                MainUI.Instance.OnInteractBuildingMenu.Invoke(buildingInGame);
            }
